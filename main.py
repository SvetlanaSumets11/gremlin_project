...
from gremlin.interfaces.entities import AuthorInterface, BookInterface, UserInterface
...
def put_book(book: Book) -> dict:
    if not AuthorInterface.is_exists(search_field='id', search_value=book.author_id):
...
def add_book(book: Book) -> dict:
    if not AuthorInterface.is_exists('id', book.author_id):
        raise ValueError(f'Author does not exist')
    return BookInterface.add_or_update_vertex(book)
...
def delete_author(author_id: str):
    if not AuthorInterface.is_exists('id', author_id):
        raise ValueError(f'Author does not exist')
    if BookInterface.is_exists('author_id', author_id):
...
def delete_user(user_id: str):
    if not UserInterface.is_exists('id', user_id):
...
def form_user_actions(relation: str, user_id: str, book_id: str, days: int = None):
    if not UserInterface.is_exists(search_field='id', search_value=user_id):
        raise ValueError(f'User does not exist')
